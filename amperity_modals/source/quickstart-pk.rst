...
To keep a customer's Amperity ID stable, PKs are used to recognize that a given source row changed and was not deleted.
...
* **first_name** is an example of a bad PK field because it is unlikely to be *unique* among all rows in a table
* **updated_at** is another example of a bad PK field because even though it is unique, its value for a given row can *change*, making it unstable
* **customer_id** is an example of a good PK field because it is likely to identify the same source row if the contents of the row changes, tomorrow