...
* For phone numbers: ensure that only fields with actual phone numbers are added to the PII table, along with consistent formatting. Any phone number that is empty or does not contain a value is not added.
* For states: ensure that only values that are two characters, such as "WA", "NY", or "CA", are added to the PII table. Any state that is not two characters is not added.
