...
A merge joins together paths in a conditional or percent split so that users do not need to duplicate logic if the journey for both paths should be the same after the split. 
