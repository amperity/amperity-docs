...
#. Add a SQL table (using |sql_spark|) to the customer 360 database that selects PII fields from the **Customer 360** table, along with any other fields Bluecore requires, and then outputs them to a table with columns that map to the required naming patterns.
