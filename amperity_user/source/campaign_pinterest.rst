...
To make your `product catalog available as a data source <https://help.pinterest.com/en/business/article/data-source-ingestion>`__ |ext_link| to |destination-name|, for the purpose of enabling Pins, configure an SFTP server with the file that has the product catalog in a location that is accessible as a URL by a user-agent.
