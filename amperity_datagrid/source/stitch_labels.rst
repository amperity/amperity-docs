...
* **semantic** The semantic tag associated with the value. The recommended semantic is **pk**, as this is the Amperity ID for individual customer records. The **pk** value must be unique within the Stitch labels table.
  ...
  .. note:: You may use other semantic values for Stitch labels, such as **email** or a foreign key, as long as the :ref:`associated blocking strategy is configured for Stitch <configure-stitch-general-clustering-blocking>`.
