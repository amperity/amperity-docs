...
.. include:: ../../amperity_datagrid/source/table_stitch_blocklistvalues.rst
...
.. include:: ../../amperity_datagrid/source/table_unified_preprocessed_raw.rst
...
.. qa-stitch-look-for-foreign-keys-prioritization-sk-start

Many tenants use separation keys to split records that do not match. In some use cases separation key unmatching must be prioritized over foreign key matching. In this situation, :ref:`configure Stitch to prioritize separation keys over foreign keys <configure-stitch-general-clustering-matching-strategy>`.

.. qa-stitch-look-for-foreign-keys-prioritization-sk-end
...
#. If a trivial duplicate is identified, :ref:`configure Stitch to use a semantic exclusion <configure-stitch-advanced-profile-trivial-duplicates-exclusions>`.

.. qa-stitch-look-for-foreign-keys-trivial-duplicates-what-end

.. include:: ../../amperity_datagrid/source/configure_stitch.rst
   :start-after: .. configure-stitch-advanced-profile-trivial-duplicates-warning-start
   :end-before: .. configure-stitch-advanced-profile-trivial-duplicates-warning-end
...
Nicknames can affect the results of blocking and clustering when they prevent obvious matches from being matched. For example, Mike is a nickname for Michael. :ref:`Blocking strategies <configure-stitch-general-clustering-blocking>` that include **given-name** use the first three characters to match records.

In this example, "Mik" and "Mic" do not match, but :ref:`depending the presence of foreign keys <configure-stitch-general-clustering-matching-strategy>` and other profile values (like **email**, **surname**, **phone**, and **address**), these records may still be scored together.
...
.. TODO: REMOVE THE NOTE IN THE WEAKEST MATCH TOPIC, MOVE TO HERE. THEN LOOK FOR WAYS TO INCLUDE BACK.
...
#. Consider tuning Stitch to :ref:`raise or lower the pairwise comparison scoring threshold <configure-stitch-general-clustering-matching-thresholds>`. The default threshold is 3.0.
...
#. Determine if your :ref:`foreign key matching vs. separation key unmatching <configure-stitch-general-clustering-matching-strategy>` strategy needs to be modified. 
...
#. Were :ref:`blocking size limits <qa-stitch-look-for-blocking-keys>` exceeded? Does your tenant have the right set of :ref:`blocking strategies <configure-stitch-general-clustering-blocking>` configured?
#. Are :ref:`trivial duplicates <qa-stitch-look-for-foreign-keys-trivial-duplicates>` present?
